...
class FareTransfer extends Contract {
    ...
    async EnrollCustomer(ctx, id, firstName, lastName, primaryTransit ){
        ...
        if (exists){
            throw new Error(`The customer ${id} already exist`);
        }
    ...
    async UpdatePrimaryTransit(ctx, id, newTransitId, firstName, lastName) {
    //     // Your code goes here
        const validTransit = await utils.isValidTransit(newTransitId);
        if (!validTransit) {
            throw new Error(`The transitId ${newTransitId} is not valid`);
        }
        ...
        return ctx.stub.putState(newTransitId, Buffer.from(stringify(sortKeysRecursive(customer))));
    ...
    async GetAllCustomers(ctx) {
        ...
        while(!result.done){
            const strValue = Buffer.from(result.value.value.toString()).toString('utf');
            let record;
            try{
